...
const ScolarReclamations = () => {
    ...
    const viewDetails = async (reclamation) => {
        try {
            const details = await reclamationService.getReclamation(reclamation.public_id || reclamation.id)
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            setUpdating(true)
            await reclamationService.updateReclamation(selectedReclamation.public_id || selectedReclamation.id, {
                status: newStatus,
    ...
    const getStatusIcon = (status) => {
        switch (status?.toUpperCase()) {
            case 'NOUVEAU':
            case 'PENDING': return <FiAlertCircle style={{ color: '#3b82f6' }} />
            case 'EN_COURS':
            case 'IN_PROGRESS': return <FiClock style={{ color: '#f59e0b' }} />
            case 'RESOLU':
            case 'RESOLVED': return <FiCheckCircle style={{ color: '#10b981' }} />
            case 'REJETE':
            case 'REJECTED': return <FiX style={{ color: '#ef4444' }} />
            default: return null
        }
    }
    ...
    const getStatusBadgeStyle = (status) => {
        ...
        return styles[statusUpper] || { background: '#f3f4f6', color: '#6b7280' }
    }
    ...
    const getRoleBadge = (role) => {
        const roleConfig = {
            ...
            'scolar_administrator': { label: 'Scolar', icon: FiUser, color: '#06b6d4', bg: 'rgba(6, 182, 212, 0.1)' },
    ...
    const getSenderInfo = (rec) => {
        ...
        if (rec.user_name) {
            return { name: rec.user_name, email: rec.user_email || null, role: rec.user_role || null }
        }
    ...
    const stats = {
        ...
        nouveau: reclamations.filter(r => ['NOUVEAU', 'pending'].includes(r.status)).length,
        en_cours: reclamations.filter(r => ['EN_COURS', 'in_progress'].includes(r.status)).length,
        resolu: reclamations.filter(r => ['RESOLU', 'resolved'].includes(r.status)).length
    }
        ...
        <div className="dashboard-page">
            ...
            <div style={{ marginBottom: '2rem' }}>
                ...
                <p style={{ color: 'var(--text-muted)', margin: '0.5rem 0 0 0' }}>
                    Traitement des réclamations des étudiants
                </p>
            </div>
            ...
            <div style={{
                ...
                <div style={{ background: 'var(--bg-card)', padding: '1rem', borderRadius: '12px', textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
            ...
            {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--bg-card)', borderRadius: '12px', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                                        ...
                                        >
                                            ...
                                            <td style={{ padding: '1rem' }}>
                                                <span style={{
                                                    ...
                                                    {rec.category || rec.category_name || 'N/A'}
                                                </span>
                                            </td>
                            ...
                            <div style={{ padding: '1.5rem', overflow: 'auto', flex: 1 }}>
                                        ...
                                        return (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                <div style={{
                                                    ...
                                                    height: '50px',
                                                    borderRadius: '50%',
                                                    background: roleBadge ? roleBadge.bg : '#06b6d4',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                                    justifyContent: 'center',
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                                    <div>
                                        ...
                                        <span style={{
                                            ...
                                            {selectedReclamation.category || selectedReclamation.category_name || 'N/A'}
                                        </span>
                                    </div>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>
                                ...
                                <div style={{ marginBottom: '1.5rem' }}>
                                    ...
                                    <div style={{
                                        ...
                                        {selectedReclamation.description || selectedReclamation.title || 'Pas de description'}
                                    </div>
                                </div>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ marginBottom: '1rem' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                            {['NOUVEAU', 'EN_COURS', 'RESOLU', 'REJETE'].map(status => (
                                                <button
                                                    key={status}
                                                    ...
                                                    style={{
                                                        padding: '0.5rem 1rem',
                                                        borderRadius: '8px',
                                                        border: newStatus === status ? '2px solid #06b6d4' : '1px solid #e2e8f0',
                                                        background: newStatus === status ? '#ffffff' : '#f8fafc',
                                                        cursor: 'pointer',
                                                        fontWeight: newStatus === status ? '600' : '400',
                                                        color: '#1e293b',
                                                        display: 'flex',
                                                        alignItems: 'center',
                                                        gap: '0.35rem'
...
export default ScolarReclamations
