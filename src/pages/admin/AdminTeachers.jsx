...
import {
  ...
  FiUser, FiMail, FiCheck, FiXCircle, FiBook,
  FiAlertTriangle, FiRefreshCw, FiLock
} from 'react-icons/fi'
...
const AdminTeachers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingUser) {
        ...
        await userService.createUser({ ...formData, role: 'teacher' })
  ...
  const toggleActive = async (user) => {
    try {
      ...
      toast.success(`Enseignant ${user.is_active ? 'désactivé' : 'activé'}`)
      loadTeachers()
    } catch (error) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <FiBook /> Gestion des Enseignants
          </h2>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--bg-card)', borderRadius: '12px' }}>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            {searchQuery ? 'Aucun résultat pour cette recherche' : 'Commencez par ajouter un enseignant'}
          </p>
        </div>
      ) : (
        <div style={{
          ...
          {filteredTeachers.map((teacher, index) => (
            <motion.div
              ...
              style={{
                ...
                borderRadius: '16px',
                padding: '1.5rem',
                border: `2px solid ${teacher.is_active ? 'transparent' : 'rgba(239, 68, 68, 0.3)'}`,
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{
                    ...
                    {teacher.first_name?.[0] || teacher.email?.[0]?.toUpperCase() || 'P'}
                  </div>
                  <div>
              ...
              <div style={{
                ...
                📅 Inscrit le {new Date(teacher.date_joined).toLocaleDateString('fr-FR')}
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    border: 'none',
                    background: teacher.is_active ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)',
                    color: teacher.is_active ? '#ef4444' : '#10b981',
                    cursor: 'pointer'
                  }}
                  title={teacher.is_active ? 'Désactiver' : 'Activer'}
                >
                  {teacher.is_active ? <FiXCircle /> : <FiCheck />}
                </button>
                <button
          ...
          >
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h3 style={{ margin: 0 }}>{editingUser ? '✏️ Modifier Enseignant' : '➕ Nouvel Enseignant'}</h3>
